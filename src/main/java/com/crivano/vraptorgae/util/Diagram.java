...
public class Diagram {
	...
	public void addClassNSuper(Class clazz, int numSuperclasses) {
		Element e = new Element(clazz);
		l.put(clazz.getName(), e);
		...
		while (clazz.getSuperclass() != null && numSuperclasses != 0) {
			clazz = clazz.getSuperclass();
			if (clazz.equals(Object.class))
				break;
			if (!l.containsKey(clazz.getName()))
	...
	public void createGraphML(String filename, boolean fIncludeAttributes,
		...
		//
		for (Element e : l.values()) {
			boolean persistent = ObjectifyModel.class.isAssignableFrom(e.clazz);
			boolean enumerable = Enum.class.isAssignableFrom(e.clazz);
			String fillColor = persistent ? "#D0FFD0" : (enumerable ? "#D0D0FF"
					: "#FFFFFF");
			...
			if ((e.fIncludeAttributes != null) ? e.fIncludeAttributes
				...
				for (int i = 0; i < f.length; i++) {
					if ("serialVersionUID".equals(f[i].getName()))
						continue;
					if (!fFirst)
						writer.write("\n");
					fFirst = false;
		...
		for (Element e : l.values()) {
			...
			for (int i = 0; i < f.length; i++) {
				...
				} else if (com.googlecode.objectify.Key.class
						.isAssignableFrom(f[i].getType())) {
